...
def run_main():
    ...
    print('step 1')
    target_dir = 'Data'
    data_filename = 'bead_0_no_transformation_sampled.xyz'
    scanning_robot_tmatrix_filename = 'scanning_robot_transformation_matrix.txt'
    welding_robot_tmatrix_filename = 'welding_robot_transformation_matrix.txt'
    data = np.genfromtxt(os.path.join(target_dir,data_filename), delimiter=' ', skip_header=0, usecols=(0, 1, 2))

    # combine scanning robot transformation and welding robot transformations by pre-multiplication
    T = np.loadtxt(os.path.join(target_dir, welding_robot_tmatrix_filename)) @ np.loadtxt(os.path.join(target_dir, scanning_robot_tmatrix_filename))

    raw = do_transform(data, T)
    ...
    # get a slice of the bead 'seg_t' thick centred on the centroid
    seg_t = 0.2
    ...
    # update transformation matrix (pre multiplication)
    ...
    # end of gen_fc_bead
    #select the plate dimensions by altering plate_params
    print('DEBUG')
    print(inter_x, y_e, popt, [30, 10, 1])
    gen_fc_bead(inter_x, y_e, popt, [30, 10, 1], trans, 'fc_bead_gen.py')


    print('Running freecad macro')
    try:
